...
function computeTooltipPosition(
  ...
  if (placement === 'center') {
    return {
      top: Math.max(
        margin,
        Math.min(vh / 2 - tooltipHeight / 2, vh - tooltipHeight - margin),
      ),
      left: Math.max(
        margin,
        Math.min(vw / 2 - tooltipWidth / 2, vw - tooltipWidth - margin),
      ),
    }
  }
...
export function LearnTour() {
  ...
  const visibleStepNumber = TOUR_STEPS.slice(0, stepIndex + 1).filter(
    isStepTargetVisible,
  ).length
